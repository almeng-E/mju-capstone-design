...
for index, row in df.iterrows():
    ...
    try:
        ...
        if pd.isna(row['tags']) or row['tags'] == "N/A":
            ...
            combined_list = [item for sublist in res.values() for item in (sublist if isinstance(sublist, list) else [sublist])]
...
df.to_csv('tagged_videos4.csv', index=False)
print("Processing complete. Data saved to 'tagged_videos4.csv'")
...
# # Process batch of data with blank check and refined logic
...
#             tags = df.at[row_index, 'tags']  # Get current 'tags' value for this row
            
#             # # Check for empty or NaN transcript
#             # if pd.isna(transcript) or not str(transcript).strip():
#             #     df.at[row_index, 'tags'] = 'No Transcript'
#             #     print(f"Row {row_index}: No Transcript")
#             # Process rows if 'tags' is 'N/A' or empty
#             if pd.isna(tags) or tags == "N/A":
#                 filtered_inputs.append(input_data)
#                 row_indices.append(row_index)

#         # Process rows that passed the filtering
#         if filtered_inputs:
